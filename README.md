...
## View information and games at https://simplegameengine.org
...
This code is mostly HTML boilerplate code, however, there are two things to note in it. If you look between the ```<body>``` tags you will notice two ```<script>``` tags that bring in two different JavaScript files. The first we will create in the next steps and the second is the ```main.js``` file you just created. They are both now linked together!

Ensure you have Node.js downloaded, here is the download link if you do not (https://nodejs.org/en/download/current).

Open up the terminal or command line either in the code editor or on your computer. Navigate to your project folder and run the following commands. 
...
If you are on Windows and this returns an error make sure you are in the command prompt and not Powershell.
...
This uses the Simple Game Engine to create a new game. The ```loop: loop``` is specifying the main game loop function that will run for every frame of the game. The code put inside the loop function will run every frame. So let's create that now!
...
This code does two things. The first is that we have created a function called ```loop```. This is the function that we passed to the game when we said ```loop: loop```. The second thing this code does is start the game with ```game.begin()```.
...
Simple Game Engine uses a consistent method for passing properties to objects. Let's take the ```Game``` class for example. To pass properties to it you will use curly brackets ```{}```. Inside these curly brackets will be the property name followed by the value. For example.
...
The ```color``` property takes a string representing a valid CSS color name, HEX code, or RGB value, and will set the item's color to the given value.
...
The ```name``` property defaults to ```"Simple Game Engine"```. In this case, acts a bit differently. It will set the browser title to the given value and if an HTML tag is present with the id of ```engine-game-name``` it will set that text to be the same. If no value is given it defaults to ```"Simple Game Engine"```
...
The ```key()``` method returns a boolean representing if the given key is pressed. It takes a string of a JS key name as a parameter.
...
The ```move()``` method will move the object by the given amount. 
...
If you only want to move the object on one axis, X for example, you could completely remove the ```y```property and leave only the ```x```property.
...
The ```bar()``` function will create a new UI bar element. This will have a border with a certain amount inside filled in, something like a classic health bar. Here is an example with all of the possible properties.
...
The bar will need to be updated every frame by specifying the name of the bar. Here is an example that would update the bar given above. The name property must match the name of the target bar.
...
The ```number()``` function will create a new UI number element. This will be a simple text that shows a number. Here is an example with all of the possible properties.
...
The number will need to be updated every frame by specifying the name of the number. Here is an example that would update the number given above. The name property must match the name of the target number
...
The ```friction``` property will set the amount of friction (how fast the pieces will slow down) the pieces will have.

The ```dur``` short for the duration will determine how long the pieces will be shown.
...
The ```engine.EngineMath``` class can be used to do the math and generate random numbers.
...
This is coming soon, however, it can be used.
...
The following are some general engine functions that can be used to accomplish goals.

```engine.route()``` can be used to route an ```engine.Object``` to another ```engine.Object```. Here is an example with all customer properties.
...
This will move the ```enemy``` object toward the ```player``` object. 
...
The ```type``` property specifies the routing that should be used. It can be set to ```"axis"``` to route ```object1``` along the axis and 45 deg angles or set to ```"trig"``` to route ```object1``` using trigonometry. 


```engine.collide()```

The ```engine.collide()``` function can be used to check for collision between an ```engine.Object``` to another ```engine.Object```. Here is an example with all custom properties.
...
This will return ```true```if the two objects are colliding and ```false``` if they are not.
